...
def draw_lottery_winning_numbers(selected: set) -> set:
    ...
    Шанс определяет, будет ли у игрока хотя бы 1 совпадение (победа) или совпадений не будет вовсе (проигрыш);
    при этом само число совпадений при победе остаётся случайным."""
    winning = set(random.sample(range(1, LOTTERY_NUMBERS_TOTAL + 1), LOTTERY_WIN_COUNT))
    natural_win = len(selected & winning) > 0
    forced_win = apply_chance("лотерея", natural_win)
    if forced_win == natural_win:
        return winning
    rest_pool = [n for n in range(1, LOTTERY_NUMBERS_TOTAL + 1) if n not in selected]
    if forced_win:
        hit = random.choice(list(selected))
        rest = random.sample(rest_pool, LOTTERY_WIN_COUNT - 1)
        return set([hit] + rest)
    return set(random.sample(rest_pool, LOTTERY_WIN_COUNT))
