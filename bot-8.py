...
def fmt(n: int) -> str:
    """Сокращает число: 1000 -> 1к, 10000 -> 10к, 1000000 -> 1кк, 2000000 -> 2кк.
    Дробная часть показывается, только если она не нулевая (1.5к, но не 1.0к)."""
    n = int(n)
    sign = "-" if n < 0 else ""
    n = abs(n)

    def _short(value: int, divisor: int, suffix: str) -> str:
    ...
    if n >= 1_000_000:
        return f"{sign}{_short(n, 1_000_000, 'кк')}"
    if n >= 1_000:
        return f"{sign}{_short(n, 1_000, 'к')}"
    return f"{sign}{n}"
