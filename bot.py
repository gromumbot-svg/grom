...
def apply_defaults(d: dict) -> dict:
    ...
    d.setdefault("x50_history", [])  # список последних выпадений цвета Х50
...
GAME_KEYS = [
    "рулетка", "мины", "блэкджек", "краш", "башня", "монетка", "hilo", "квак",
    "футбол", "баскетбол", "боулинг", "дартс", "слоты", "лимбо", "кости", "скачки",
    "х50", "плинко",
]
# Исход этих игр решает настоящий дайс Telegram, либо чистый рандом без возможности подкрутки — шанс не настраивается
CHANCE_ADJUSTABLE_GAMES = [
    g for g in GAME_KEYS
    if g not in ("краш", "футбол", "баскетбол", "боулинг", "дартс", "слоты", "кости", "х50", "плинко")
] + ["лотерея"]
GAME_LABELS = {
    "рулетка": "🎰 Рулетка",
    "мины": "💣 Мины",
...
def fmt(n: int) -> str:
    ...
    1000 -> 1 000, 10000 -> 10 000, 100000 -> 100 000 и т.д."""
    n = int(n)
    sign = "-" if n < 0 else ""
    n = abs(n)
...
def is_game_frozen(game_key: str):
    """Система компаний отключена — игры больше никогда не замораживаются."""
    return False, None


def apply_company_economy(game_key: str, won: bool, bet_amount: int, payout_amount: int = 0):
    """Система компаний отключена — экономика игр компаний больше не применяется."""
    return
...
def parse_amount_str(s: str) -> Optional[int]:
    """Парсит сумму из строки: поддерживает обычные числа (10000) и
    сокращения (10к -> 10000, 1.5к -> 1500, 1кк -> 1000000)."""
    s = s.strip().lower().replace(",", ".")
    m = re.match(r"^(\d+(?:\.\d+)?)\s*(кк|к)?$", s)
    if not m:
        return None
...
def set_bank_balance(user_id: int, amount: int):
    uid = str(user_id)
    data["bank"][uid] = amount
    save_data(data)


def add_bank_balance(user_id: int, amount: int):
    new_bal = max(0, get_bank_balance(user_id) + amount)
    set_bank_balance(user_id, new_bal)
...
def parse_bet(text: str) -> Optional[Tuple[int, str, any, float]]:
    ...
    if len(parts) < 2:
        return None
    amount = parse_amount_str(parts[0])
    if amount is None:
        return None
...
def main_keyboard():
    return [
        ...
        [Button.text("💬 Чаты", resize=True), Button.text("🎮 Игры", resize=True)],
        [Button.text("🎁 Бонус", resize=True)],
    ]


MENU_BUTTON_TEXTS = {
    "👤 Профиль", "📋 Команды", "💬 Чаты", "🎮 Игры",
    "🎁 Бонус", "🔮 Хогвартс",
    "Политика", "Изменить язык",
}

# Регулярки всех «прямых» команд/триггеров бота (кроме самого ввода числа).
# Нужны, чтобы понять: пользователь не вводит запрошенное число, а начал
# что-то другое — значит, ожидание числа нужно снять, а не ругаться на него.
OTHER_COMMAND_REGEXES = [
    re.compile(r"^п(\s|$)"),
    re.compile(r"^дуэль\s+"),
    re.compile(r"^забанить\s+\S+"),
    re.compile(r"^разбанить\s+\S+"),
    re.compile(r"^мины\s+\d+(?:\.\d+)?(?:кк|к)?(\s+\d+)?$"),
    re.compile(r"^(бж|бл(э|е)к(джек)?)\s+\d+(?:\.\d+)?(?:кк|к)?$"),
    re.compile(r"^краш\s+\d+(?:\.\d+)?(?:кк|к)?(\s+\d+(\.\d+)?)?$"),
    re.compile(r"^башня\s+\d+(?:\.\d+)?(?:кк|к)?$"),
    re.compile(r"^квак\s+\d+(?:\.\d+)?(?:кк|к)?$"),
    re.compile(r"^(хило|хл|hl|hilo)\s+\d+(?:\.\d+)?(?:кк|к)?$"),
    re.compile(r"^футбол\s+\d+(?:\.\d+)?(?:кк|к)?\s+(гол|мимо)$"),
    re.compile(r"^(баскетбол|баскет)\s+\d+(?:\.\d+)?(?:кк|к)?\s+(гол|мимо)$"),
    re.compile(r"^боулинг\s+\d+(?:\.\d+)?(?:кк|к)?\s+(страйк|мимо)$"),
    re.compile(r"^дартс\s+\d+(?:\.\d+)?(?:кк|к)?\s+(яблочко|мимо)$"),
    re.compile(r"^слоты\s+\d+(?:\.\d+)?(?:кк|к)?$"),
    re.compile(r"^лимбо\s+\d+(?:\.\d+)?(?:кк|к)?\s+\d+(\.\d+)?$"),
    re.compile(r"^кости\s+\d+(?:\.\d+)?(?:кк|к)?\s+(чет|чёт|нечет|нечёт)$"),
    re.compile(r"^скачки\s+\d+(?:\.\d+)?(?:кк|к)?\s+[1-5]$"),
    re.compile(r"^монетк?а\s+\d+(?:\.\d+)?(?:кк|к)?\s+(орел|орёл|решка|о|р)$"),
    re.compile(r"^лотерея\s+\d+(?:\.\d+)?(?:кк|к)?$"),
    re.compile(r"^х50\s+\d+(?:\.\d+)?(?:кк|к)?\s+(ч|ф|к|з)$"),
    re.compile(r"^плинко\s+\d+(?:\.\d+)?(?:кк|к)?$"),
    re.compile(r"^дроп(\s|$)"),
    re.compile(r"^создать клан\s+.+"),
    re.compile(r"^вступить\s+.+"),
    re.compile(r"^вклад\s+\d+(?:\.\d+)?(?:кк|к)?$"),
    re.compile(r"^фаст\s+\d+(?:\.\d+)?(?:кк|к)?\s+\d+$"),
    re.compile(r"^активировать\s+\S+$"),
]
OTHER_COMMAND_EXACT = {"б", "b", "в", "принять", "топ", "лог", "ставки", "отмена", "го", "go", "клан", "бонус", "игры"}



def is_other_command_text(text: Optional[str]) -> bool:
    """True, если текст — это не ответ на запрошенное число/данные,
    а какая-то другая кнопка/команда бота. В этом случае ожидание ввода
    нужно снять и не ругаться «Введите сумму целым числом»."""
    if not text:
        return False
    t = text.strip()
    if not t:
    ...
    if t in MENU_BUTTON_TEXTS:
        return True
    tl = norm(t.lower())
    if tl in OTHER_COMMAND_EXACT:
        return True
    for rx in OTHER_COMMAND_REGEXES:
        if rx.match(tl):
            return True
    return False
...
def looks_numeric(text: str) -> bool:
    """True, если текст похож на ПОПЫТКУ ввести число (цифры, пробелы-разделители,
    запятые, необязательный минус) — а не на другую команду/кнопку.
    Используется, чтобы отличить опечатку в числе от того, что человек передумал
    и нажал что-то совсем другое."""
    if not text:
        return False
...
def parse_amount_input(text: str) -> Optional[int]:
    """Парсит число, разрешая пробелы/неразрывные пробелы/запятые как разделители тысяч
    (например «1 000» или «1,000» тоже засчитываются как 1000)."""
    if not text:
        return None
    cleaned = text.strip().replace(" ", "").replace("\u00a0", "").replace(",", "")
    try:
...
@client.on(events.CallbackQuery(
    ...
    ["upload_gif", "give_gram", "take_gram",
     "chance_game", "reset_user", "reset_all",
...
async def admin_callbacks(event):
    ...
    if action == "upload_gif":
        ...
        await event.respond(
            ...
            "Введите ID пользователя (или @username). Обнулится его личный баланс.",
            parse_mode="html",
        )
        await event.answer()
        ...
        await event.respond(
            ...
            "Это обнулит балансы <b>всех игроков</b> без возможности отмены.",
            buttons=[[
...
async def restore_data_start(event):
    ...
    await event.respond(
        ...
        "⚠️ Это полностью заменит текущие данные бота (балансы, кланы и т.д.). "
...
async def set_game_chance_handler(event):
    ...
    if not game_key:
        return
    if is_other_command_text(event.raw_text):
...
async def reset_all_decision(event):
    ...
    await event.edit("💣 Все балансы игроков обнулены.")
    await event.answer()
...
def clear_user_active_games(user_id: int):
    ...
    lottery_games.pop(user_id, None)
...
# нажал любую ДРУГУЮ кнопку — соответствующее ожидание сбрасывается. Кнопки, которые сами
# ЗАПУСКАЮТ ожидание для КОНКРЕТНОГО состояния, исключены — но только для ЭТОГО состояния,
# а не для всех разом (иначе два независимых флоу могут повиснуть друг на друге одновременно).
ADMIN_FLOW_CONTINUATION_EXACT = {
    "give_gram", "take_gram", "reset_user", "create_check",
    "ban_user_start", "unban_user_start", "add_admin_start", "create_cheque_start",
}
ADMIN_FLOW_CONTINUATION_PREFIXES = ("pick_rank:",)

DATA_RESTORE_CONTINUATION_EXACT = {"restore_data_start"}
CHANCE_GAME_CONTINUATION_PREFIXES = ("chance_pick:",)
EMOJI_SLOT_CONTINUATION_PREFIXES = ("pick_emoji_slot:",)


@client.on(events.CallbackQuery(func=lambda e: bool(e.data)))
async def cancel_pending_input_on_button(event):
    uid = event.sender_id
    cb = event.data.decode()

    if cb not in ADMIN_FLOW_CONTINUATION_EXACT and not cb.startswith(ADMIN_FLOW_CONTINUATION_PREFIXES):
        admin_flow_state.pop(uid, None)
    if not cb.startswith(CHANCE_GAME_CONTINUATION_PREFIXES):
        waiting_for_chance_game.pop(uid, None)
    if not cb.startswith(EMOJI_SLOT_CONTINUATION_PREFIXES):
        waiting_for_emoji_slot.pop(uid, None)
    if cb not in DATA_RESTORE_CONTINUATION_EXACT:
...
async def admin_flow_handler(event):
    ...
    if state["action"] == "reset_user" and state["step"] == "id":
        ...
        admin_flow_state.pop(event.sender_id, None)
        await event.reply(
            f"✅ Баланс пользователя {mention(target_id, target_name)} обнулён.",
            parse_mode="html",
        )
        return
...
async def give_gram(event):
    ...
    if len(parts) < 3:
        ...
        return
    try:
        amount = parse_amount_str(parts[-1])
        target = parts[-2]
        if amount is None:
...
async def transfer_gram(event):
    ...
    if reply_msg:
        ...
        try:
            ...
            return
        amount = parse_amount_str(parts[2])
        if amount is None:
            await event.reply(
                "❗ Укажи айди и сумму числами: <code>п 123456789 1000</code>", parse_mode="html"
            )
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and norm(e.raw_text.lower().strip()) == "банк"))
async def bank_show(event):
    user_id = event.sender_id
    sender = await event.get_sender()
    name = sender.first_name if sender else str(user_id)
    ...
    await event.reply(
        ...
        f"🏦 Банк: <b>{fmt(bank_balance)}</b> GROM",
        parse_mode="html",
    )


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^банк\s+(положить|снять)\s+.+$", norm(e.raw_text.lower().strip()))))
async def bank_deposit_withdraw(event):
    user_id = event.sender_id
    sender = await event.get_sender()
    name = sender.first_name if sender else str(user_id)

    text = norm(event.raw_text.strip().lower())
    m = re.match(r"^банк\s+(положить|снять)\s+(.+)$", text)
    action = m.group(1)
    amount = parse_amount_str(m.group(2).strip())

    if amount is None:
        await event.reply("❗ Укажи сумму числом, например: <code>банк положить 10к</code>", parse_mode="html")
        return

    if action == "положить":
        balance = get_user_balance(user_id)
        if amount > balance:
            await event.reply(f"❌ Недостаточно GROM на балансе (баланс: {fmt(balance)}).", parse_mode="html")
            return
        add_balance(user_id, -amount)
        add_bank_balance(user_id, amount)
        await event.reply(
            f"🏦 Положено в банк: <b>{fmt(amount)} GROM</b>\n"
            f"💰 Баланс: {fmt(get_user_balance(user_id))} GROM\n"
            f"🏦 Банк: {fmt(get_bank_balance(user_id))} GROM",
            parse_mode="html",
        )
    else:
        bank_balance = get_bank_balance(user_id)
        if amount > bank_balance:
            await event.reply(f"❌ Недостаточно GROM в банке (в банке: {fmt(bank_balance)}).", parse_mode="html")
            return
        add_bank_balance(user_id, -amount)
        add_balance(user_id, amount)
        await event.reply(
            f"🏦 Снято из банка: <b>{fmt(amount)} GROM</b>\n"
            f"💰 Баланс: {fmt(get_user_balance(user_id))} GROM\n"
            f"🏦 Банк: {fmt(get_bank_balance(user_id))} GROM",
            parse_mode="html",
        )
...
def looks_like_bet(text: str) -> bool:
    ...
    if len(parts) < 2:
        return False
    return parse_amount_str(parts[0]) is not None
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^мины\s+\d+(?:\.\d+)?(?:кк|к)?(\s+\d+)?$", e.raw_text.lower().strip())))
async def start_mines(event):
    ...
    bet = parse_amount_str(parts[1])
    mines_count = int(parts[2]) if len(parts) > 2 else 5

    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>мины 10к</code>", parse_mode="html")
        return
    if bet <= 0:
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^(бж|бл(э|е)к(джек)?)\s+\d+(?:\.\d+)?(?:кк|к)?$", e.raw_text.lower().strip())))
async def start_blackjack(event):
    ...
    bet = parse_amount_str(event.raw_text.strip().split()[1])
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>блэк 10к</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    if bj_hand_value(player) == 21:
        ...
        if win:
            ...
            blackjack_games.pop(user_id, None)
            await event.reply(
                bj_text(game, reveal_dealer=True, result_line=f"🎉 <b>Блэкджек!</b> Выигрыш: <b>{fmt(win_amount)} GROM</b>"),
                parse_mode="html",
            )
        else:
            ...
            blackjack_games.pop(user_id, None)
            await event.reply(
                bj_text(game, reveal_dealer=True, result_line=f"❌ <b>Проигрыш.</b> -<b>{fmt(bet)} GROM</b>"),
                parse_mode="html",
            )
        return
...
async def bj_finish(event, owner_id: int, game: dict):
    ...
    if natural_result != "push":
        ...
        if win:
            ...
            add_balance(owner_id, win_amount)
            apply_company_economy("блэкджек", True, bet, win_amount)
            result = f"🎉 <b>Победа!</b> Выигрыш: <b>{fmt(win_amount)} GROM</b>"
        else:
            apply_company_economy("блэкджек", False, bet)
            result = f"❌ <b>Проигрыш.</b> -<b>{fmt(bet)} GROM</b>"
    else:
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^краш\s+\d+(?:\.\d+)?(?:кк|к)?(\s+\d+(\.\d+)?)?$", e.raw_text.lower().strip())))
async def start_crash(event):
    ...
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>краш 10к</code>", parse_mode="html")
        return
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^башня\s+\d+(?:\.\d+)?(?:кк|к)?$", e.raw_text.lower().strip())))
async def start_tower(event):
    ...
    bet = parse_amount_str(event.raw_text.strip().split()[1])
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>башня 10к</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^квак\s+\d+(?:\.\d+)?(?:кк|к)?$", e.raw_text.lower().strip())))
async def start_kvak(event):
    ...
    bet = parse_amount_str(event.raw_text.strip().split()[1])
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>квак 10к</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^(хило|хл|hl|hilo)\s+\d+(?:\.\d+)?(?:кк|к)?$", norm(e.raw_text.lower().strip()))))
async def start_hilo(event):
    ...
    bet = parse_amount_str(event.raw_text.strip().split()[1])
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>hilo 10к</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
...
FOOTBALL_GOAL_MULT = 1.6
FOOTBALL_MISS_MULT = 2.4
BASKETBALL_GOAL_MULT = 2.4
BASKETBALL_MISS_MULT = 1.6

# Значения кубика Telegram, которые считаются «голом»:
# ⚽ — 3, 4 и 5 (на анимации мяч влетает в ворота); 🏀 — только 4 и 5.
DICE_GOAL_VALUES = {
    "футбол": {3, 4, 5},
    "баскетбол": {4, 5},
}
DICE_MULTS = {
    "футбол": (FOOTBALL_GOAL_MULT, FOOTBALL_MISS_MULT),
    "баскетбол": (BASKETBALL_GOAL_MULT, BASKETBALL_MISS_MULT),
}
...
async def play_dice_sport(event, game_key: str, emoji: str, choice: str, bet: int):
    ...
    goal_values = DICE_GOAL_VALUES.get(game_key, {4, 5})
    actual_result = "гол" if value in goal_values else "мимо"
    await asyncio.sleep(4)  # даём анимации доиграть перед итогом

    won = (choice == actual_result)
    goal_mult, miss_mult = DICE_MULTS.get(game_key, (FOOTBALL_GOAL_MULT, FOOTBALL_MISS_MULT))
    mult = goal_mult if choice == "гол" else miss_mult
    ...
    if won:
        ...
        await event.reply(
            f"{emoji} <b>{actual_result.upper()}</b>\n\n"
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^футбол\s+\d+(?:\.\d+)?(?:кк|к)?\s+(гол|мимо)$", norm(e.raw_text.lower().strip()))))
async def play_football(event):
    ...
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>футбол 10к гол</code>", parse_mode="html")
        return
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^(баскетбол|баскет)\s+\d+(?:\.\d+)?(?:кк|к)?\s+(гол|мимо)$", norm(e.raw_text.lower().strip()))))
async def play_basketball(event):
    ...
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>баскетбол 10к гол</code>", parse_mode="html")
        return
...
# ========== ИГРЫ: БОУЛИНГ / ДАРТС (настоящий бросок Telegram, подкрутить невозможно) ==========
# 🎳 боулинг и 🎯 дартс — дайсы Telegram со значениями 1-6.
BOWLING_WIN_MULT = 2.88   # страйк: значения 5-6 (2/6)
BOWLING_LOSE_MULT = 1.44  # мимо: значения 1-4 (4/6)
DARTS_WIN_MULT = 2.88     # яблочко: значения 5-6 (2/6)
DARTS_LOSE_MULT = 1.44    # мимо: значения 1-4 (4/6)
...
DICE6_MULTS = {
    "боулинг": (BOWLING_WIN_MULT, BOWLING_LOSE_MULT),
    "дартс": (DARTS_WIN_MULT, DARTS_LOSE_MULT),
}
DICE6_WIN_LABEL = {"боулинг": "страйк", "дартс": "яблочко"}


async def play_dice6(event, game_key: str, emoji: str, choice: str, bet: int):
    user_id = event.sender_id
    win_label = DICE6_WIN_LABEL[game_key]

    dice_msg = await client.send_message(event.chat_id, file=InputMediaDice(emoticon=emoji))
    value = dice_msg.media.value
    win_values = DICE6_WIN_VALUES[game_key]
    actual_result = win_label if value in win_values else "мимо"
    await asyncio.sleep(4)

    won = (choice == actual_result)
    win_mult, lose_mult = DICE6_MULTS[game_key]
    mult = win_mult if choice == win_label else lose_mult

    if won:
        payout = int(bet * mult)
        add_balance(user_id, payout)
        await event.reply(
            f"{emoji} <b>{actual_result.upper()}</b> ({value})\n\n"
            f"✅ Угадал! Выигрыш: <b>{fmt(payout)} GROM</b> (x{mult})",
            parse_mode="html",
        )
    else:
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^боулинг\s+\d+(?:\.\d+)?(?:кк|к)?\s+(страйк|мимо)$", norm(e.raw_text.lower().strip()))))
async def play_bowling(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = parse_amount_str(parts[1])
    choice = parts[2]
    ...
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>боулинг 10к страйк</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    await play_dice6(event, "боулинг", "🎳", choice, bet)


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^дартс\s+\d+(?:\.\d+)?(?:кк|к)?\s+(яблочко|мимо)$", norm(e.raw_text.lower().strip()))))
async def play_darts(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = parse_amount_str(parts[1])
    choice = parts[2]
    ...
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>дартс 10к яблочко</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    await play_dice6(event, "дартс", "🎯", choice, bet)


# ========== ИГРА: СЛОТЫ (настоящий слот-дайс Telegram, подкрутить невозможно) ==========
# Дайс 🎰 в Telegram выдаёт значение 1-64. По официально известной раскладке:
# 64 = три семёрки (777, джекпот), 1/22/43 = другие тройки (bar/grape/lemon).
SLOTS_JACKPOT_VALUE = 64
SLOTS_TRIPLE_VALUES = {1, 22, 43}
SLOTS_JACKPOT_MULT = 36
SLOTS_TRIPLE_MULT = 8


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^слоты\s+\d+(?:\.\d+)?(?:кк|к)?$", norm(e.raw_text.lower().strip()))))
async def play_slots(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = parse_amount_str(parts[1])

    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>слоты 10к</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    dice_msg = await client.send_message(event.chat_id, file=InputMediaDice(emoticon="🎰"))
    value = dice_msg.media.value
    await asyncio.sleep(3)

    if value == SLOTS_JACKPOT_VALUE:
        payout = int(bet * SLOTS_JACKPOT_MULT)
        add_balance(user_id, payout)
        await event.reply(
            f"🎰 <b>777! ДЖЕКПОТ!</b>\n\n✅ Выигрыш: <b>{fmt(payout)} GROM</b> (x{SLOTS_JACKPOT_MULT})",
            parse_mode="html",
        )
    elif value in SLOTS_TRIPLE_VALUES:
        payout = int(bet * SLOTS_TRIPLE_MULT)
        add_balance(user_id, payout)
        await event.reply(
            f"🎰 <b>Тройная комбинация!</b>\n\n✅ Выигрыш: <b>{fmt(payout)} GROM</b> (x{SLOTS_TRIPLE_MULT})",
            parse_mode="html",
        )
    else:
        await event.reply(
            f"🎰 Комбинация не сыграла.\n\n❌ Проигрыш: <b>{fmt(bet)} GROM</b>",
            parse_mode="html",
        )


# ========== ИГРА: ЛИМБО ==========
# Мгновенный раунд без анимации: сервер сам решает "крашпоинт" по формуле краш-игр
# и сравнивает его с заказанным множителем. Шанс настраивается админом (не дайс Telegram).
LIMBO_HOUSE_EDGE = 0.04


def roll_limbo_crash_point() -> float:
    r = random.random()
    if r < LIMBO_HOUSE_EDGE:
        return 1.0
    return round((1 - LIMBO_HOUSE_EDGE) / (1 - r), 2)


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^лимбо\s+\d+(?:\.\d+)?(?:кк|к)?\s+\d+(\.\d+)?$", norm(e.raw_text.lower().strip()))))
async def play_limbo(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = parse_amount_str(parts[1])
    ...
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>лимбо 10к 5</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    if target < 1.01:
        await event.reply("❗ Множитель должен быть не меньше 1.01.")
        return
    if get_user_balance(user_id) < bet:
        await event.reply("Недостаточно GROM на балансе")
        return
    ...
    add_balance(user_id, -bet)
    crash_point = roll_limbo_crash_point()
    natural_win = crash_point >= target
    won = apply_chance("лимбо", natural_win)

    if won:
        payout = int(bet * target)
        add_balance(user_id, payout)
        await event.reply(
            f"📈 Лимбо: дошло до <b>x{crash_point}</b>\n\n"
            f"✅ Твой множитель x{target} сыграл! Выигрыш: <b>{fmt(payout)} GROM</b>",
            parse_mode="html",
        )
    else:
        shown_point = crash_point if crash_point < target else round(target - 0.01, 2)
        await event.reply(
            f"📈 Лимбо: дошло до <b>x{shown_point}</b>\n\n"
            f"❌ Не дошло до x{target}. Проигрыш: <b>{fmt(bet)} GROM</b>",
            parse_mode="html",
        )


# ========== ИГРА: КОСТИ ЧЁТ/НЕЧЕТ (два настоящих дайса Telegram) ==========
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^кости\s+\d+(?:\.\d+)?(?:кк|к)?\s+(чет|чёт|нечет|нечёт)$", norm(e.raw_text.lower().strip()))))
async def play_dice_parity(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = parse_amount_str(parts[1])
    choice = "чет" if parts[2] in ("чет", "чёт") else "нечет"

    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>кости 10к чет</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    dice1_msg = await client.send_message(event.chat_id, file=InputMediaDice(emoticon="🎲"))
    dice2_msg = await client.send_message(event.chat_id, file=InputMediaDice(emoticon="🎲"))
    v1, v2 = dice1_msg.media.value, dice2_msg.media.value
    total = v1 + v2
    actual = "чет" if total % 2 == 0 else "нечет"
    await asyncio.sleep(4)
    ...
    if won:
        ...
        add_balance(user_id, payout)
        await event.reply(
            f"🎲 {v1} + 🎲 {v2} = <b>{total}</b> ({actual})\n\n"
            f"✅ Победа! Выигрыш: <b>{fmt(payout)} GROM</b>",
            parse_mode="html",
        )
    else:
        await event.reply(
            f"🎲 {v1} + 🎲 {v2} = <b>{total}</b> ({actual})\n\n"
            f"❌ Проигрыш: <b>{fmt(bet)} GROM</b>",
            parse_mode="html",
        )


# ========== ИГРА: СКАЧКИ ==========
# Вес — вероятность победы лошади в процентах, mult подобран так, чтобы EV = 96% при честной игре.
HORSES = [
    (1, "🐎 Гром", 40, 2.4),
    (2, "🐴 Ветер", 25, 3.84),
    (3, "🐎 Молния", 15, 6.4),
...
HORSES_BY_NUM = {h[0]: h for h in HORSES}


def roll_horse_winner() -> int:
    roll = random.randint(1, 100)
    acc = 0
    for num, name, weight, mult in HORSES:
        acc += weight
        if roll <= acc:
            return num
    return HORSES[-1][0]
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^скачки\s+\d+(?:\.\d+)?(?:кк|к)?\s+[1-5]$", norm(e.raw_text.lower().strip()))))
async def play_horses(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = parse_amount_str(parts[1])
    picked = int(parts[2])
    ...
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>скачки 10к 1</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    await event.reply("🐎 Забег начался...")
    await asyncio.sleep(2)

    natural_winner = roll_horse_winner()
    natural_win = (natural_winner == picked)
    won = apply_chance("скачки", natural_win)

    if won:
        winner_num = picked
    elif natural_winner == picked:
        # админ подкрутил результат в минус игроку — подменяем победителя на другую лошадь
        winner_num = next(h[0] for h in HORSES if h[0] != picked)
    else:
        winner_num = natural_winner

    _, winner_name, _, winner_mult = HORSES_BY_NUM[winner_num]
    result_lines = "\n".join(f"{h[1]} (x{h[3]})" + (" 🏆" if h[0] == winner_num else "") for h in HORSES)

    if won:
        payout = int(bet * winner_mult)
        add_balance(user_id, payout)
        await event.reply(
            f"🏁 <b>Финиш!</b>\n\n{result_lines}\n\n"
            f"✅ Твоя лошадь {HORSES_BY_NUM[picked][1]} победила! Выигрыш: <b>{fmt(payout)} GROM</b>",
            parse_mode="html",
        )
    else:
        await event.reply(
            f"🏁 <b>Финиш!</b>\n\n{result_lines}\n\n"
            f"❌ Твоя лошадь {HORSES_BY_NUM[picked][1]} не победила. Проигрыш: <b>{fmt(bet)} GROM</b>",
            parse_mode="html",
        )


# ========== ИГРА: Х50 (цветная рулетка, чистый рандом, подкрутить нельзя) ==========
X50_COLORS = {
    "ч": {"name": "чёрный", "emoji": "⚫", "mult": 2, "weight": 475},
    "ф": {"name": "фиолетовый", "emoji": "🟣", "mult": 3, "weight": 317},
    "к": {"name": "красный", "emoji": "🔴", "mult": 5, "weight": 190},
    "з": {"name": "зелёный", "emoji": "🟢", "mult": 50, "weight": 18},
}
X50_ROUND_WAIT = 13  # секунд с первой ставки в раунде до автоспина

x50_bets: dict = {}          # chat_id -> list of (user_id, name, amount, color)
x50_first_bet_time: dict = {}
x50_spinning: dict = {}
x50_last_bet: dict = {}      # (chat_id, user_id) -> list of (amount, color)
_x50_locks: dict = {}


def get_x50_lock(chat_id: int) -> asyncio.Lock:
    if chat_id not in _x50_locks:
        _x50_locks[chat_id] = asyncio.Lock()
    return _x50_locks[chat_id]


def roll_x50_color() -> str:
    roll = random.randint(1, 1000)
    acc = 0
    for code, info in X50_COLORS.items():
        acc += info["weight"]
        if roll <= acc:
            return code
    return "ч"
...
async def resolve_x50_round(chat_id: int):
    bets = x50_bets.pop(chat_id, [])
    x50_first_bet_time.pop(chat_id, None)

    winner_color = roll_x50_color()
    info = X50_COLORS[winner_color]

    data["x50_history"].append({
        "color": winner_color, "mult": info["mult"], "at": datetime.now().isoformat(),
    })
    data["x50_history"] = data["x50_history"][-200:]
    save_data(data)

    groups: dict = {}
    for user_id, name, amount, color in bets:
        groups.setdefault(color, []).append((user_id, name, amount))

    blocks = [f"🎡 <b>Рулетка X50:</b> {info['emoji']} <u>x{info['mult']}</u>"]
    for code, cinfo in X50_COLORS.items():
        if code not in groups:
            continue
        lines = []
        for user_id, name, amount in groups[code]:
            if code == winner_color:
                payout = int(amount * cinfo["mult"])
                add_balance(user_id, payout)
                lines.append(f"💸 {mention(user_id, name)} -- ставка <b>{fmt(amount)}</b> → <b>{fmt(payout)} GROM</b>")
            else:
                lines.append(f"❌ {mention(user_id, name)} -- ставка <b>{fmt(amount)}</b> → <b>0 GROM</b>")
        lines_joined = "\n".join(lines)
        blocks.append(f"{cinfo['emoji']} <u>Ставки на x{cinfo['mult']}:</u>\n<blockquote>{lines_joined}</blockquote>")

    x50_spinning[chat_id] = False

    await send_chunks(
        chat_id, "\n\n".join(blocks),
        buttons=[[Button.inline("🔁 Повторить", data=f"x50rpt:{chat_id}")]],
    )


async def x50_round_timer(chat_id: int):
    await asyncio.sleep(X50_ROUND_WAIT)
    async with get_x50_lock(chat_id):
        if x50_spinning.get(chat_id):
            return
        if not x50_bets.get(chat_id):
            x50_first_bet_time.pop(chat_id, None)
            return
        x50_spinning[chat_id] = True
    await resolve_x50_round(chat_id)


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^х50\s+\d+(?:\.\d+)?(?:кк|к)?\s+(ч|ф|к|з)$", norm(e.raw_text.lower().strip()))))
async def play_x50_bet(event):
    chat_id = event.chat_id
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    amount = parse_amount_str(parts[1])
    ...
    if amount is None:
        await event.reply("❗ Укажи ставку числом, например: <code>х50 10к ч</code>", parse_mode="html")
        return
    if amount <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    async with get_x50_lock(chat_id):
        if x50_spinning.get(chat_id):
            await event.reply("⏳ Раунд уже крутится — подожди результат.")
            return
        ...
        add_balance(user_id, -amount)
        x50_bets.setdefault(chat_id, []).append((user_id, name, amount, color))
        x50_last_bet.setdefault((chat_id, user_id), []).append((amount, color))

        is_first = chat_id not in x50_first_bet_time
        if is_first:
            x50_first_bet_time[chat_id] = datetime.now()

    info = X50_COLORS[color]
    await event.reply(
        f"{info['emoji']} {mention(user_id, name)} поставил <b>{fmt(amount)} GROM</b> на x{info['mult']}",
        parse_mode="html",
    )

    if is_first:
        asyncio.create_task(x50_round_timer(chat_id))


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode().startswith("x50rpt:")))
async def x50_repeat_callback(event):
    chat_id = int(event.data.decode().split(":", 1)[1])
    user_id = event.sender_id

    saved = x50_last_bet.get((chat_id, user_id))
    if not saved:
        await event.answer("Ты не играл в прошлом раунде", alert=True)
        return

    total_cost = sum(a for a, _ in saved)
    ...
    async with get_x50_lock(chat_id):
        if x50_spinning.get(chat_id):
            await event.answer("Раунд уже крутится — подожди.", alert=True)
            return
        add_balance(user_id, -total_cost)
        x50_bets.setdefault(chat_id, []).extend((user_id, name, a, c) for a, c in saved)
        is_first = chat_id not in x50_first_bet_time
        if is_first:
            x50_first_bet_time[chat_id] = datetime.now()

    for amount, color in saved:
        info = X50_COLORS[color]
        await event.respond(
            f"{info['emoji']} {mention(user_id, name)} поставил <b>{fmt(amount)} GROM</b> на x{info['mult']}",
            parse_mode="html",
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and norm(e.raw_text.lower().strip()) == "дроп"))
async def x50_drop_history(event):
    history = data.get("x50_history", [])[-20:][::-1]
    if not history:
        await event.reply("История выпадений пуста.")
        return
    lines = ["📜 <b>Последние выпадения Х50</b>\n"]
    for h in history:
        info = X50_COLORS.get(h["color"])
        emoji = info["emoji"] if info else "⚪"
        lines.append(f"{emoji} x{h['mult']} — {h['at'][:16].replace('T', ' ')}")
    await event.reply("\n".join(lines), parse_mode="html")


# ========== ИГРА: ПЛИНКО ==========
# Биномиальное распределение (как в настоящем плинко): края редкие и дают крупный
# множитель, центр частый и даёт меньше единицы. EV ≈ 0.95 при честной игре.
PLINKO_ROWS = 6
PLINKO_SLOT_MULTS = [3.0, 1.5, 0.9, 0.5, 0.9, 1.5, 3.0]  # 7 слотов, EV ≈ 0.95


def plinko_roll_slot() -> int:
    return sum(1 for _ in range(PLINKO_ROWS) if random.random() < 0.5)


def plinko_render(position: float) -> str:
    ball_row = "".join(
        f"{'●' if round(position) == i else '○':^5}"
        for i in range(len(PLINKO_SLOT_MULTS))
    )
    mult_row = "".join(f"{f'{m:g}':^5}" for m in PLINKO_SLOT_MULTS)
    return ball_row + "\n" + mult_row


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^плинко\s+\d+(?:\.\d+)?(?:кк|к)?$", norm(e.raw_text.lower().strip()))))
async def play_plinko(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = parse_amount_str(parts[1])

    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>плинко 10к</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    final_slot = plinko_roll_slot()
    center = (len(PLINKO_SLOT_MULTS) - 1) / 2

    msg = await event.reply(
        f"🔻 <b>Плинко:</b> ставка {fmt(bet)} GROM\n\n<code>{plinko_render(center)}</code>",
        parse_mode="html",
    )

    for step in range(1, PLINKO_ROWS + 1):
        pos = center + (final_slot - center) * step / PLINKO_ROWS
        await asyncio.sleep(0.45)
        try:
            await msg.edit(
                f"🔻 <b>Плинко:</b> ставка {fmt(bet)} GROM\n\n<code>{plinko_render(pos)}</code>",
                parse_mode="html",
            )
        except Exception:
            pass

    mult = PLINKO_SLOT_MULTS[final_slot]
    payout = int(bet * mult)
    add_balance(user_id, payout)
    await asyncio.sleep(0.3)

    if mult >= 1:
        result_line = f"✅ Шарик упал в слот <b>x{mult}</b>! Выигрыш: <b>{fmt(payout)} GROM</b>"
    else:
        result_line = f"❌ Шарик упал в слот <b>x{mult}</b>. Выплата: <b>{fmt(payout)} GROM</b> из ставки {fmt(bet)} GROM"

    try:
        await msg.edit(
            f"🔻 <b>Плинко:</b> ставка {fmt(bet)} GROM\n\n<code>{plinko_render(final_slot)}</code>\n\n{result_line}",
            parse_mode="html",
        )
    except Exception:
        pass
...
# ========== ИГРА: МОНЕТКА ==========
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^монетк?а\s+\d+(?:\.\d+)?(?:кк|к)?\s+(орел|орёл|решка|о|р)$", norm(e.raw_text.lower().strip()))))
async def play_coin(event):
    ...
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>монета 10к орёл</code>", parse_mode="html")
        return
...
# ========== ИГРА: ЛОТЕРЕЯ ==========
LOTTERY_NUMBERS_TOTAL = 90
LOTTERY_PICK_COUNT = 13
LOTTERY_WIN_COUNT = 8
LOTTERY_COOLDOWN_HOURS = 5
LOTTERY_MULTIPLIERS = {8: 500, 7: 100, 6: 50, 5: 5, 4: 3, 3: 2, 2: 1.5, 1: 1.5, 0: 0}
lottery_games: dict = {}  # user_id -> {"bet", "chat_id", "selected": set(), "message_id"}


def can_play_lottery(user_id: int) -> Tuple[bool, Optional[int]]:
    if is_staff(user_id):
        return True, None
    last = data["lottery_cooldown"].get(str(user_id))
    if not last:
        return True, None
    last_time = datetime.fromisoformat(last)
    if datetime.now() >= last_time + timedelta(hours=LOTTERY_COOLDOWN_HOURS):
        return True, None
    remaining = int((last_time + timedelta(hours=LOTTERY_COOLDOWN_HOURS) - datetime.now()).total_seconds())
    return False, remaining


def set_lottery_played(user_id: int):
    if not is_staff(user_id):
        data["lottery_cooldown"][str(user_id)] = datetime.now().isoformat()
        save_data(data)


def lottery_keyboard(game: dict, finished: bool = False, winning: Optional[set] = None) -> list:
    rows = []
    for r in range(9):
        row = []
        for c in range(10):
            n = r * 10 + c + 1
            if finished:
                if n in winning and n in game["selected"]:
                    label = f"✅{n}"
                elif n in winning:
                    label = f"🎯{n}"
                elif n in game["selected"]:
                    label = f"❌{n}"
                else:
                    label = str(n)
                row.append(Button.inline(label, data="lot_done"))
            else:
                label = f"✅{n}" if n in game["selected"] else str(n)
                row.append(Button.inline(label, data=f"lot_pick:{game['owner']}:{n}"))
        rows.append(row)
    return rows
...
def lottery_text(
    game: dict,
    finished: bool = False,
    winning: Optional[set] = None,
    matches: Optional[int] = None,
    win_amount: Optional[int] = None,
) -> str:
    header = f"🎰 <b>ЛОТЕРЕЯ</b>\n\nСтавка: <b>{fmt(game['bet'])} GROM</b>\n"
    if not finished:
        chosen = ", ".join(str(n) for n in sorted(game["selected"])) or "—"
        return (
            header
            + f"Выбери <b>{LOTTERY_PICK_COUNT}</b> чисел от 1 до 90.\n"
            + f"Выбрано ({len(game['selected'])}/{LOTTERY_PICK_COUNT}): {chosen}"
        )
    win_nums = ", ".join(str(n) for n in sorted(winning))
    mult = LOTTERY_MULTIPLIERS.get(matches, 0)
    result_line = (
        f"🎉 Выигрыш: <b>{fmt(win_amount)} GROM</b> (x{mult})"
        if win_amount
        else "❌ Выигрыш: <b>0 GROM</b>"
    )
    return (
        header
        + f"🎯 Выигрышные числа: <b>{win_nums}</b>\n"
        + f"✅ Совпадений: <b>{matches}/{LOTTERY_WIN_COUNT}</b>\n\n"
        + result_line
    )


def build_lottery_winning_with_matches(selected: set, target_matches: int) -> set:
    """Собирает 8 выигрышных чисел так, чтобы ровно target_matches из них совпали с выбором игрока."""
    target_matches = max(0, min(target_matches, LOTTERY_WIN_COUNT, len(selected)))
    hits = random.sample(list(selected), target_matches) if target_matches > 0 else []
    non_selected_pool = [n for n in range(1, LOTTERY_NUMBERS_TOTAL + 1) if n not in selected]
    misses = random.sample(non_selected_pool, LOTTERY_WIN_COUNT - target_matches)
    return set(hits + misses)


def draw_lottery_winning_numbers(selected: set) -> set:
    """Разыгрывает 8 выигрышных чисел, применяя настроенный админом шанс игры «лотерея».
    chance < 50 — шанс на крупный выигрыш растёт (при 0% — гарантированные 8/8 совпадений);
    chance > 50 — совпадения обрезаются (при 100% — гарантированные 0/8);
    chance = 50 — честный случайный розыгрыш."""
    chance = get_game_chance("лотерея")
    winning = set(random.sample(range(1, LOTTERY_NUMBERS_TOTAL + 1), LOTTERY_WIN_COUNT))
    if chance == 50:
        return winning

    natural_matches = len(selected & winning)
    if chance < 50:
        force_trigger_pct = (50 - chance) * 2  # 0..100
        if random.randint(1, 100) <= force_trigger_pct:
            target = round(LOTTERY_WIN_COUNT * (50 - chance) / 50)  # chance=0 -> 8, chance=49 -> ~0
            if target > natural_matches:
                return build_lottery_winning_with_matches(selected, target)
    else:
        force_trigger_pct = (chance - 50) * 2
        if random.randint(1, 100) <= force_trigger_pct:
            cap = round(LOTTERY_WIN_COUNT * (100 - chance) / 50)  # chance=100 -> 0, chance=51 -> ~8
            if cap < natural_matches:
                return build_lottery_winning_with_matches(selected, cap)
    return winning


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^лотерея\s+\d+(?:\.\d+)?(?:кк|к)?$", e.raw_text.lower().strip())))
async def start_lottery(event):
    user_id = event.sender_id
    if user_id in lottery_games:
        await event.reply("❗ У тебя уже есть активная игра в лотерею. Заверши её сначала.")
        return

    can_play, remaining = can_play_lottery(user_id)
    if not can_play:
        hours = remaining // 3600
        minutes = (remaining % 3600) // 60
        await event.reply(f"⏳ Лотерея доступна раз в {LOTTERY_COOLDOWN_HOURS} часов. Осталось: {hours}ч {minutes}мин.")
        return

    bet = parse_amount_str(event.raw_text.strip().split()[1])
    if bet is None:
        await event.reply("❗ Укажи ставку числом, например: <code>лотерея 10к</code>", parse_mode="html")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    set_lottery_played(user_id)
    game = {"owner": user_id, "chat_id": event.chat_id, "bet": bet, "selected": set()}
    lottery_games[user_id] = game

    msg = await event.reply(lottery_text(game), buttons=lottery_keyboard(game), parse_mode="html")
    game["message_id"] = msg.id


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode().startswith("lot_pick:")))
async def lottery_pick(event):
    _, owner_str, n_str = event.data.decode().split(":")
    owner_id = int(owner_str)
    n = int(n_str)
    ...
    game = lottery_games.get(owner_id)
    if not game:
        await event.answer("Игра уже завершена.", alert=True)
        return
    ...
    if n in selected:
        selected.discard(n)
        await event.edit(lottery_text(game), buttons=lottery_keyboard(game), parse_mode="html")
        await event.answer()
        return

    if len(selected) >= LOTTERY_PICK_COUNT:
        await event.answer(f"Уже выбрано {LOTTERY_PICK_COUNT} чисел.", alert=True)
        return
    ...
    winning = draw_lottery_winning_numbers(selected)
    matches = len(selected & winning)
    mult = LOTTERY_MULTIPLIERS.get(matches, 0)
    win_amount = int(game["bet"] * mult)
    if win_amount > 0:
        add_balance(owner_id, win_amount)
    lottery_games.pop(owner_id, None)

    await event.edit(
        lottery_text(game, finished=True, winning=winning, matches=matches, win_amount=win_amount),
        buttons=lottery_keyboard(game, finished=True, winning=winning),
        parse_mode="html",
    )
    if win_amount > 0:
        await event.answer(f"🎉 Совпадений: {matches}/{LOTTERY_WIN_COUNT}! Выигрыш: {fmt(win_amount)} GROM", alert=True)
    else:
        await event.answer(f"😔 Совпадений: {matches}/{LOTTERY_WIN_COUNT}. Проигрыш.", alert=True)


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "lot_done"))
async def lottery_done_noop(event):
    await event.answer()
...
HELP_TEXT = (
    ...
    "💡 Любую сумму можно писать сокращённо: <code>10к</code> = 10 000, <code>1.5кк</code> = 1 500 000\n\n"
    ...
    "<b>🏦 Банк</b>\n"
    "• <code>банк</code> — посмотреть баланс банка\n"
    "• <code>банк положить [сумма]</code> — положить GROM в банк\n"
    "• <code>банк снять [сумма]</code> — снять GROM из банка\n\n"
    "<b>🎰 Рулетка</b>\n"
    ...
    "<b>🎳 Боулинг / 🎯 Дартс</b>\n"
    "• <code>боулинг [ставка] страйк/мимо</code> — настоящий бросок Telegram, подкрутить нельзя\n"
    "• <code>дартс [ставка] яблочко/мимо</code> — то же самое, дротиком\n\n"
    "<b>🎰 Слоты</b>\n"
    "• <code>слоты [ставка]</code> — настоящий слот-дайс Telegram, три семёрки или другая тройка — выигрыш\n\n"
    "<b>📈 Лимбо</b>\n"
    "• <code>лимбо [ставка] [множитель]</code> — мгновенный раунд без анимации, например "
    "<code>лимбо 500 5</code> — выигрыш x5, если дошло\n\n"
    "<b>🎲 Кости чёт/нечет</b>\n"
    "• <code>кости [ставка] чет/нечет</code> — сумма двух костей чётная или нечётная (x1.92)\n\n"
    "<b>🐎 Скачки</b>\n"
    "• <code>скачки [ставка] [номер лошади]</code> — 5 лошадей с разными коэффициентами, номера 1–5\n\n"
    "<b>🎡 Рулетка X50</b>\n"
    "• <code>х50 [ставка] [цвет]</code> — ч (x2), ф (x3), к (x5), з (x50). Чистый рандом, подкрутить нельзя.\n"
    "• <code>дроп</code> — история последних выпадений X50\n\n"
    "<b>🔻 Плинко</b>\n"
    "• <code>плинко [ставка]</code> — шарик падает через 6 рядов колышков в один из 7 слотов (от x0.5 до x3.0)\n\n"
    "<b>🎰 Лотерея</b>\n"
    "• <code>лотерея [ставка]</code> — выбери 13 чисел из 90, разыгрываются 8 выигрышных. "
    "Доступно раз в 5 часов (админам — без ограничений)\n\n"
    "<b>⚔️ Дуэли</b>\n"
    ...
    "• <code>/check [сумма] [активации]</code> — создать чек-ссылку со своего баланса, например <code>/check 5000 5</code>, скинь её кому угодно\n"
...
async def profile_button(event):
    ...
    await event.reply(
        ...
        f"{emoji_tag('id', '🆔')} ID: `{user_id}`{house_line}{clan_line}\n\n"
...
async def games_button(event):
    await event.reply(
        ...
        "🎳 <b>Боулинг</b> / 🎯 <b>Дартс</b>\n"
        "Тоже настоящий бросок Telegram — подкрутить невозможно! Угадай страйк/яблочко или мимо.\n"
        "Команда: <code>боулинг 500 страйк</code> или <code>дартс 500 яблочко</code>\n\n"
        "🎰 <b>Слоты</b>\n"
        "Настоящий слот-дайс Telegram. Три семёрки — джекпот x36, любая другая тройка — x8.\n"
        "Команда: <code>слоты 500</code>\n\n"
        "📈 <b>Лимбо</b>\n"
        "Мгновенный раунд без анимации — назови множитель, бот сразу скажет, дошло ли до него.\n"
        "Команда: <code>лимбо 500 5</code>\n\n"
        "🎲 <b>Кости чёт/нечет</b>\n"
        "Сумма двух костей — чётная или нечётная, выигрыш x1.92.\n"
        "Команда: <code>кости 500 чет</code>\n\n"
        "🐎 <b>Скачки</b>\n"
        "5 лошадей с разными коэффициентами (от x2.4 до x12). Ставь на номер.\n"
        "Команда: <code>скачки 500 1</code>\n\n"
        "🎡 <b>Рулетка X50</b>\n"
        "Ставь на цвет: ч (x2), ф (x3), к (x5), з (x50). Чистый рандом, подкрутить нельзя.\n"
        "Команда: <code>х50 500 ч</code>. История выпадений — команда <code>дроп</code>\n\n"
        "🔻 <b>Плинко</b>\n"
        "Шарик падает через колышки в один из 7 слотов — от x0.5 в центре до x3.0 по краям.\n"
        "Команда: <code>плинко 500</code>\n\n"
        "⚔️ <b>Дуэль</b>\n"
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and e.raw_text.lower().strip() == "игры"))
async def games_list_command(event):
    lines = [GAME_LABELS[g] for g in GAME_KEYS] + [GAME_LABELS["лотерея"], "⚔️ Дуэль"]
    await event.reply(
        "🎮 <b>Список игр</b>\n\n" + "\n".join(lines),
        parse_mode="html",
    )
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^вклад\s+\d+(?:\.\d+)?(?:кк|к)?$", e.raw_text.lower().strip())))
async def clan_deposit(event):
    ...
    amount = parse_amount_str(parts[1])
    if amount is None:
        await event.reply("❌ Укажи сумму числом, например: <code>вклад 10к</code>", parse_mode="html")
        return
    if amount <= 0:
        await event.reply("❌ Сумма должна быть больше 0.")
        return
...
async def create_promo_cmd(event):
    ...
    try:
        ...
        await event.reply("❗ Количество активаций должно быть числом.")
        return
    if amount is None:
        await event.reply("❗ Сумма должна быть числом, например: <code>10к</code>.")
        return
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^фаст\s+\d+(?:\.\d+)?(?:кк|к)?\s+\d+$", norm(e.raw_text.lower().strip()))))
async def start_fast(event):
    ...
    total = int(parts[2])
    if amount is None:
        await event.reply("❗ Укажи сумму числом, например: <code>фаст 10к 5</code>", parse_mode="html")
        return
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^/check(@\w+)?\s+\d+(?:\.\d+)?(?:кк|к)?\s+\d+\s*$", e.raw_text.strip().lower())))
async def check_command(event):
    """/check [сумма] [активации] — создаёт чек-ссылку прямо командой, доступно любому игроку."""
    user_id = event.sender_id
    parts = event.raw_text.strip().lower().split()
    amount = parse_amount_str(parts[1])
    uses = int(parts[2])

    if amount is None:
        await event.reply("❗ Укажи сумму числом, например: <code>/check 10к 5</code>", parse_mode="html")
        return
    if amount <= 0:
        await event.reply("❗ Сумма должна быть больше 0.")
        return
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^/check(@\w+)?\s*$", e.raw_text.strip())))
async def check_command_help(event):
    await event.reply(
        "🧾 <b>Создание чека</b>\n\n"
        "Формат: <code>/check сумма активации</code>\n"
        "Например: <code>/check 5000 5</code> — чек на 5 000 GROM с 5 активациями "
        "(спишется 25 000 GROM, каждый из 5 разных людей получит по 5 000 GROM по ссылке).",
        parse_mode="html",
    )
