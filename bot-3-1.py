...
# Исход этих игр решает настоящий дайс Telegram (или он равновероятен по своей природе) — шанс не настраивается
CHANCE_ADJUSTABLE_GAMES = [
    g for g in GAME_KEYS
    if g not in ("краш", "футбол", "баскетбол", "боулинг", "дартс", "слоты", "кости")
] + ["лотерея"]
GAME_LABELS = {
    "рулетка": "🎰 Рулетка",
    "мины": "💣 Мины",
...
OTHER_COMMAND_REGEXES = [
    ...
    re.compile(r"^мины\s+\d+(\s+\d+)?$"),
    re.compile(r"^(бж|бл(э|е)к(джек)?)\s+\d+$"),
    re.compile(r"^краш\s+\d+(\s+\d+(\.\d+)?)?$"),
    re.compile(r"^башня\s+\d+$"),
    re.compile(r"^квак\s+\d+$"),
    re.compile(r"^(хило|хл|hl|hilo)\s+\d+$"),
    re.compile(r"^футбол\s+\d+\s+(гол|мимо)$"),
    re.compile(r"^(баскетбол|баскет)\s+\d+\s+(гол|мимо)$"),
    re.compile(r"^боулинг\s+\d+\s+(страйк|мимо)$"),
    re.compile(r"^дартс\s+\d+\s+(яблочко|мимо)$"),
    re.compile(r"^слоты\s+\d+$"),
    re.compile(r"^лимбо\s+\d+\s+\d+(\.\d+)?$"),
    re.compile(r"^кости\s+\d+\s+(чет|чёт|нечет|нечёт)$"),
    re.compile(r"^скачки\s+\d+\s+[1-5]$"),
    re.compile(r"^монетк?а\s+\d+\s+(орел|орёл|решка|о|р)$"),
    re.compile(r"^лотерея\s+\d+$"),
    re.compile(r"^создать клан\s+.+"),
    re.compile(r"^вступить\s+.+"),
    re.compile(r"^вклад\s+\d+$"),
    re.compile(r"^фаст\s+\d+\s+\d+$"),
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^боулинг\s+\d+\s+(страйк|мимо)$", norm(e.raw_text.lower().strip()))))
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^дартс\s+\d+\s+(яблочко|мимо)$", norm(e.raw_text.lower().strip()))))
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^слоты\s+\d+$", norm(e.raw_text.lower().strip()))))
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^лимбо\s+\d+\s+\d+(\.\d+)?$", norm(e.raw_text.lower().strip()))))
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^кости\s+\d+\s+(чет|чёт|нечет|нечёт)$", norm(e.raw_text.lower().strip()))))
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^скачки\s+\d+\s+[1-5]$", norm(e.raw_text.lower().strip()))))
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^лотерея\s+\d+$", e.raw_text.lower().strip())))
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^/check(@\w+)?\s+\d+\s+\d+\s*$", e.raw_text.strip())))
