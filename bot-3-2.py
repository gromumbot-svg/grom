    ...
    for code, cinfo in X50_COLORS.items():
        ...
        for user_id, name, amount in groups[code]:
            if code == winner_color:
                ...
                add_balance(user_id, payout)
                lines.append(f"💸 {mention(user_id, name)} -- ставка {fmt(amount)} → {fmt(payout)} GROM")
            else:
                lines.append(f"❌ {mention(user_id, name)} -- ставка {fmt(amount)} → 0 GROM")
        lines_joined = "\n".join(lines)
...
async def play_x50_bet(event):
    ...
    await event.reply(
        f"{info['emoji']} {mention(user_id, name)} поставил {fmt(amount)} GROM на x{info['mult']}",
        parse_mode="html",
    )
...
# множитель, центр частый и даёт меньше единицы. EV ≈ 0.93 при честной игре.
PLINKO_ROWS = 8
PLINKO_SLOT_MULTS = [6.6, 2.5, 1.2, 0.8, 0.4, 0.8, 1.2, 2.5, 6.6]  # 9 слотов
...
def plinko_render(position: float) -> str:
    cells = ["🔴" if round(position) == i else "⚪" for i in range(len(PLINKO_SLOT_MULTS))]
    mults = "  ".join(f"x{m}" for m in PLINKO_SLOT_MULTS)
    return "  ".join(cells) + "\n" + mults
...
HELP_TEXT = (
    ...
    "<b>🔻 Плинко</b>\n"
    "• <code>плинко [ставка]</code> — шарик падает через 8 рядов колышков в один из 9 слотов (от x0.4 до x6.6)\n\n"
    "<b>🎰 Лотерея</b>\n"
...
async def games_button(event):
    await event.reply(
        ...
        "🔻 <b>Плинко</b>\n"
        "Шарик падает через колышки в один из 9 слотов — от x0.4 в центре до x6.6 по краям.\n"
