...
def main_keyboard():
    return [
        ...
        [Button.text("🎁 Бонус", resize=True), Button.text("🏢 Компания", resize=True)],
        [Button.text("🧾 Чек", resize=True)],
    ]


MENU_BUTTON_TEXTS = {
    "👤 Профиль", "📋 Команды", "💬 Чаты", "🎮 Игры",
    "🎁 Бонус", "🏢 Компания", "🧾 Чек", "🔮 Хогвартс",
...
OTHER_COMMAND_REGEXES = [
    ...
    re.compile(r"^пополнить компани[юи]\s+\d+$"),
    re.compile(r"^вывод компании\s+\d+$"),
    re.compile(r"^фаст\s+\d+\s+\d+$"),
    re.compile(r"^активировать\s+\S+$"),
]
OTHER_COMMAND_EXACT = {"б", "b", "в", "принять", "топ", "лог", "ставки", "отмена", "го", "go", "клан", "бонус"}
