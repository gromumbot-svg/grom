...
CHANCE_ADJUSTABLE_GAMES = [g for g in GAME_KEYS if g not in ("краш", "футбол", "баскетбол")] + ["лотерея"]  # исход этих игр решает сам Telegram, шанс не настраивается
GAME_LABELS = {
    "рулетка": "🎰 Рулетка",
    "мины": "💣 Мины",
    "блэкджек": "🃏 Блэкджек",
...
def main_keyboard():
    return [
        ...
        [Button.text("🎁 Бонус", resize=True), Button.text("🧾 Чек", resize=True)],
    ]
...
async def cancel_pending_input_on_button(event):
    ...
    # У чек-ссылки пользователя нет продолжения через инлайн-кнопки вообще —
    # любое нажатие кнопки означает, что человек ушёл из этого флоу.
